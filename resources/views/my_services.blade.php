...
<script>
    window.onload = function() {
        var pdfPath = "{{ asset('storage/' . session('pdf_path')) }}";
        var a = document.createElement('a');
        a.href = pdfPath;
        a.download = '{{ basename(session('pdf_path')) }}';
    ...
    <div class="row">
        ...
        <div class="col-md-4 d-flex justify-content-center" style="margin-bottom: 50px">
            <div class="card" style="max-width: 260px">
                ...
                <div class="card-body">
                    <h5 class="card-title" style="color:black">{{$servicio->title}}</h5></a>
                    ...
                    <div class="d-flex justify-content-center">
                        <form id="solicitudForm_{{ $servicio->id }}" action="{{ route('send_solicitude') }}" method="post">
                            ...
                            <a href="{{route('show.single.service', $servicio->id)}}" class="btn btn-primary enviar-solicitud ml-3" data-userid="{{ $servicio->id }}">Acceder</a>
                        </form>
                    </div>
                </div>
            </div>
